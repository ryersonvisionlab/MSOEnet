...
with tf.device('/gpu:1'):
    ...
    for i in range(1, 40):
        input1 = cv2.imread('test_images/water_2/frame_%08d.jpeg' % (i),
                            cv2.IMREAD_GRAYSCALE).astype(np.float32) * \
            scale_factor
        input2 = cv2.imread('test_images/water_2/frame_%08d.jpeg' % (i+1),
                            cv2.IMREAD_GRAYSCALE).astype(np.float32) * \
            scale_factor
    ...
    for i in range(0, 39):
        filename = 'test_images/water_2/img_%08d.jpeg' % (i)
