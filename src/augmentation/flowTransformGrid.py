...
def flowTransformGrid(flow):
	with tf.get_default_graph().name_scope("flowTransformGrid"):
		...
		flowU = tf.reshape((flowU*2)/tf.cast(width, tf.float32),shape=tf.pack([batchSize,1,-1]))
		flowV = tf.reshape((flowV*2)/tf.cast(height, tf.float32),shape=tf.pack([batchSize,1,-1]))
		zeros = tf.zeros(shape=tf.pack([batchSize,1, height*width]))

		flowScaled = tf.concat(1,[flowU,flowV,zeros])
